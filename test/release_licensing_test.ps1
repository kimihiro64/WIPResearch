...
$testRoot = Join-Path $systemTempRoot (
  'robin-bv-explorations-release-licensing-' + [Guid]::NewGuid().ToString('N')
)
...
try {
  ...
  foreach ($required in @(
    ...
    $licensingIndex,
    (Join-Path $licensingRoot 'RobinBV-Apache-2.0.txt'),
    (Join-Path $licensingRoot 'RobinBV-LICENSING.md')
  )) {
  ...
  foreach ($required in @(
    ...
    (Join-Path $buildRoot 'licensing/RobinBV-Apache-2.0.txt'),
    (Join-Path $buildRoot 'licensing/RobinBV-LICENSING.md')
  )) {
    if (-not (Test-Path -LiteralPath $required -PathType Leaf)) {
  ...
  if (Test-Path -LiteralPath $testRoot) {
    ...
    $expectedLeaf = 'robin-bv-explorations-release-licensing-'
