import RobinBV.NumberField.Helpers.ZeroKernelLeadingExpansion
import RobinBV.NumberField.Proof.CharacterWeightedHigherRemainder
import RobinBV.NumberField.Proof.PairedDirichletCriticalBound

/-!
# Leading expansion of actual primitive Dirichlet zero kernels

Every multiplicity-carrying completed-L zero is retained. The generic
kernel estimate uses the proved inverse-square weights and actual ERH,
without a parity or self-duality restriction.
-/

set_option autoImplicit false

namespace RobinBV.NumberField

open BombieriVinogradov.SiegelWalfisz
open Complex Filter

noncomputable section

/-- Full-mass explicit leading error for an actual primitive Dirichlet
zero family, including both parities and all complex character values. -/
theorem norm_primitiveDirichletZeroKernel_leading_error_scaled_le
    {N : Nat} [NeZero N] {chi : DirichletCharacter Complex N}
    (hChi : Not (chi = 1)) (hPrimitive : chi.IsPrimitive) (hERH : DirichletERH chi)
    {n : Nat} (hn : 1 <= n) {x : Real} (hx : 1 < x) :
    norm (((x ^ ((n : Real) - 1 / 2) * Real.log x : Real) : Complex) *
      (tsum (fun p : QuadraticLZeroIndex chi =>
        Robin1984.robinZeroKernel n (quadraticLZeroValue p) x / quadraticLZeroValue p) -
        tsum (fun p : QuadraticLZeroIndex chi => zeroKernelLeadingTerm n (quadraticLZeroValue p) x))) <=
      quadraticLZeroMass chi *
        (Inv.inv (Real.log x) + (2 / ((n : Real) - 1 / 2)) * Inv.inv (Real.log x) ^ 2) :=
  norm_zeroKernel_sum_leading_error_scaled_le quadraticLZeroValue
    (fun p => quadraticLZeroValue_re_eq_half_of_dirichletERH hChi hPrimitive hERH p)
    (summable_quadraticLZeroWeight hChi hPrimitive) hn hx

/-- The complete actual Dirichlet leading-kernel error tends to zero
at critical normalization; this does not assert that the leading sum does. -/
theorem primitiveDirichletZeroKernel_leading_error_scaled_tendsto
    {N : Nat} [NeZero N] {chi : DirichletCharacter Complex N}
    (hChi : Not (chi = 1)) (hPrimitive : chi.IsPrimitive) (hERH : DirichletERH chi)
    {n : Nat} (hn : 1 <= n) :
    Tendsto (fun x : Real => ((x ^ ((n : Real) - 1 / 2) * Real.log x : Real) : Complex) *
      (tsum (fun p : QuadraticLZeroIndex chi =>
        Robin1984.robinZeroKernel n (quadraticLZeroValue p) x / quadraticLZeroValue p) -
        tsum (fun p : QuadraticLZeroIndex chi => zeroKernelLeadingTerm n (quadraticLZeroValue p) x)))
      atTop (nhds (0 : Complex)) :=
  zeroKernel_sum_leading_error_scaled_tendsto quadraticLZeroValue
    (fun p => quadraticLZeroValue_re_eq_half_of_dirichletERH hChi hPrimitive hERH p)
    (summable_quadraticLZeroWeight hChi hPrimitive) hn

/-- The actual weighted integral plus its complete zero kernel sum has
a derived lower-order bound. Both parity formulas use the actual Hadamard
constant; no extra remainder hypothesis is introduced. -/
theorem exists_primitiveDirichletWeightedIntegral_add_zeroKernel_bound
    {N : Nat} [NeZero N] {chi : DirichletCharacter Complex N}
    (hChi : Not (chi = 1)) (hPrimitive : chi.IsPrimitive) (hERH : DirichletERH chi)
    {n : Nat} (hn : 2 <= n) :
    exists A : Real, And (0 <= A) (forall x : Real, 3 <= x ->
      norm (dirichletCharacterWeightedIntegral chi n x +
        tsum (fun p : QuadraticLZeroIndex chi =>
          Robin1984.robinZeroKernel n (quadraticLZeroValue p) x / quadraticLZeroValue p)) <=
        A * x ^ (-(n : Real))) := by
  let B : Complex := logDeriv (symmetricCompletedLFunction chi) 0
  let Ce : Real := norm ((Real.log N : Complex) / 2 - B - (Real.log Real.pi : Complex) / 2 -
    (Real.eulerMascheroniConstant : Complex) / 2) + 2 * Real.log (2 * Real.pi) + Inv.inv (n : Real)
  let Co : Real := norm ((Real.log N : Complex) / 2 - B - (Real.log Real.pi : Complex) / 2 -
    (Real.eulerMascheroniConstant : Complex) / 2 + quadraticOddGammaConstant) + 1
  let C : Real := Ce + Co
  let A : Real := 1 + C * Inv.inv (Real.log 3)
  have hLogPi : 0 <= Real.log (2 * Real.pi) := Real.log_nonneg (by nlinarith [Real.pi_gt_three])
  have hCe : 0 <= Ce := by dsimp only [Ce]; positivity
  have hCo : 0 <= Co := by dsimp only [Co]; positivity
  have hC : 0 <= C := add_nonneg hCe hCo
  have hLog3 : 0 < Real.log (3 : Real) := Real.log_pos (by norm_num)
  have hA : 0 <= A := by dsimp only [A]; positivity
  refine Exists.intro A (And.intro hA ?_)
  intro x hx
  have hxOne : 1 < x := by linarith
  have hxPos : 0 < x := by linarith
  have hPower : 0 <= x ^ (-(n : Real)) := Real.rpow_nonneg hxPos.le _
  have hInvLog : Inv.inv (Real.log x) <= Inv.inv (Real.log 3) := by
    simpa only [one_div] using one_div_le_one_div_of_le hLog3 (Real.log_le_log (by norm_num) hx)
  have hRem (D : Real) (hD : 0 <= D) (hDC : D <= C) :
      D * (x ^ (-(n : Real)) * Inv.inv (Real.log x)) <=
        (C * Inv.inv (Real.log 3)) * x ^ (-(n : Real)) := by
    calc
      _ = (D * Inv.inv (Real.log x)) * x ^ (-(n : Real)) := by ring
      _ <= (D * Inv.inv (Real.log 3)) * x ^ (-(n : Real)) :=
        mul_le_mul_of_nonneg_right (mul_le_mul_of_nonneg_left hInvLog hD) hPower
      _ <= _ := mul_le_mul_of_nonneg_right
        (mul_le_mul_of_nonneg_right hDC (inv_pos.mpr hLog3).le) hPower
  have hCancel (a b : Complex) : -a + b + a = b := by ring
  cases chi.even_or_odd with
  | inl hEven =>
    rw [<- primitiveCharacterPrimePowerSum_eq_weightedIntegral chi hn hxOne,
      primitiveCharacterPrimePowerSum_eq_even_explicit hChi hPrimitive hEven hERH
        (primitiveCharacter_endpoint_isHadamardConstant hChi hPrimitive) hn hxOne, hCancel]
    apply (norm_primitiveCharacterEvenWeightedRemainder_le N _ hn (by linarith : 2 <= x)).trans
    change x ^ (-(n : Real)) + Ce * (x ^ (-(n : Real)) * Inv.inv (Real.log x)) <= _
    have hSmall := hRem Ce hCe (by dsimp only [C]; linarith)
    calc
      _ <= x ^ (-(n : Real)) + (C * Inv.inv (Real.log 3)) * x ^ (-(n : Real)) :=
        add_le_add le_rfl hSmall
      _ = _ := by dsimp only [A]; ring
  | inr hOdd =>
    rw [<- primitiveCharacterPrimePowerSum_eq_weightedIntegral chi hn hxOne,
      primitiveCharacterPrimePowerSum_eq_odd_explicit hChi hPrimitive hOdd hERH
        (primitiveCharacter_endpoint_isHadamardConstant hChi hPrimitive) hn hxOne, hCancel]
    apply (norm_primitiveCharacterOddWeightedRemainder_le N _ (by omega : 1 <= n) (by linarith : 2 <= x)).trans
    change Co * (x ^ (-(n : Real)) * Inv.inv (Real.log x)) <= _
    have hSmall := hRem Co hCo (by dsimp only [C]; linarith)
    calc
      _ <= (C * Inv.inv (Real.log 3)) * x ^ (-(n : Real)) := hSmall
      _ <= _ := by dsimp only [A]; nlinarith

/-- Every actual parity and endpoint correction vanishes at critical
normalization, with the complete zero sum still in the arithmetic identity. -/
theorem primitiveDirichletWeightedIntegral_add_zeroKernel_scaled_tendsto
    {N : Nat} [NeZero N] {chi : DirichletCharacter Complex N}
    (hChi : Not (chi = 1)) (hPrimitive : chi.IsPrimitive) (hERH : DirichletERH chi)
    {n : Nat} (hn : 2 <= n) :
    Tendsto (fun x : Real => ((x ^ ((n : Real) - 1 / 2) * Real.log x : Real) : Complex) *
      (dirichletCharacterWeightedIntegral chi n x +
        tsum (fun p : QuadraticLZeroIndex chi =>
          Robin1984.robinZeroKernel n (quadraticLZeroValue p) x / quadraticLZeroValue p)))
      atTop (nhds (0 : Complex)) := by
  choose A hA hBound using exists_primitiveDirichletWeightedIntegral_add_zeroKernel_bound hChi hPrimitive hERH hn
  have hSmall : Tendsto (fun x : Real => x ^ (-(1 / 2 : Real)) * Real.log x)
      atTop (nhds (0 : Real)) := by
    have h := (isLittleO_log_rpow_atTop (by norm_num : (0 : Real) < 1 / 2)).tendsto_div_nhds_zero
    apply h.congr'
    filter_upwards [Filter.eventually_gt_atTop (0 : Real)] with x hx
    rw [Real.rpow_neg hx.le]
    ring
  have hUpper : Tendsto (fun x : Real => A * (x ^ (-(1 / 2 : Real)) * Real.log x))
      atTop (nhds (0 : Real)) := by simpa only [mul_zero] using hSmall.const_mul A
  apply tendsto_zero_iff_norm_tendsto_zero.mpr
  apply squeeze_zero' (Filter.Eventually.of_forall (fun _ => norm_nonneg _)) _ hUpper
  filter_upwards [Filter.eventually_ge_atTop (3 : Real)] with x hx
  have hxPos : 0 < x := by linarith
  have hScale : 0 <= x ^ ((n : Real) - 1 / 2) * Real.log x :=
    mul_nonneg (Real.rpow_nonneg hxPos.le _) (Real.log_pos (by linarith : 1 < x)).le
  have hPowers : x ^ ((n : Real) - 1 / 2) * x ^ (-(n : Real)) = x ^ (-(1 / 2 : Real)) := by
    rw [<- Real.rpow_add hxPos]
    congr 1
    ring
  rw [norm_mul, Complex.norm_real, Real.norm_eq_abs, abs_of_nonneg hScale]
  apply (mul_le_mul_of_nonneg_left (hBound x hx) hScale).trans_eq
  calc
    _ = A * (x ^ ((n : Real) - 1 / 2) * x ^ (-(n : Real))) * Real.log x := by ring
    _ = _ := by rw [hPowers]; ring

/-- Actual higher-weight primitive Dirichlet arithmetic is minus the
complete leading zero series, up to a vanishing critical-scaled error. -/
theorem primitiveDirichletWeightedIntegral_add_leadingZeroSeries_scaled_tendsto
    {N : Nat} [NeZero N] {chi : DirichletCharacter Complex N}
    (hChi : Not (chi = 1)) (hPrimitive : chi.IsPrimitive) (hERH : DirichletERH chi)
    {n : Nat} (hn : 2 <= n) :
    Tendsto (fun x : Real => ((x ^ ((n : Real) - 1 / 2) * Real.log x : Real) : Complex) *
      (dirichletCharacterWeightedIntegral chi n x +
        tsum (fun p : QuadraticLZeroIndex chi => zeroKernelLeadingTerm n (quadraticLZeroValue p) x)))
      atTop (nhds (0 : Complex)) := by
  have h := (primitiveDirichletWeightedIntegral_add_zeroKernel_scaled_tendsto hChi hPrimitive hERH hn).sub
    (primitiveDirichletZeroKernel_leading_error_scaled_tendsto hChi hPrimitive hERH (by omega : 1 <= n))
  simp only [sub_zero] at h
  apply h.congr'
  filter_upwards [] with x
  ring

end

end RobinBV.NumberField
