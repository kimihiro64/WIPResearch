...
/-!
# VMVT source consumer for the max-fiber fallback

This module substitutes the unconditional VMVT source theorem into both
mean-value factors of the explicit fallback estimate. The resulting bound
retains only explicit finite degree products and the one absolute VMVT constant
supplied by the source theorem. It does not claim the sharp Theorem 24.7
exponent; that requires the separate joint fiber-energy Cauchy estimate.
-/

theorem exists_linnik_vmtech_double_sum_power_source_bound :
      ...
      norm (linnikVMTechDoubleSum alpha L X) ^
        ...
        ((L : Real) ^ (2 * (k * r) - 1)) ^ (2 * (k * r)) *
          ((Real.exp (C * (r : Real) * (k : Real) ^ 2 * Real.log k) *
              (X : Real) ^ linnikVMVTExponent k r) ^ (2 * (k * r)) *
  ...
  have hbase := linnik_vmtech_double_sum_power_le_explicit
...
theorem linnik_vmtech_le_root_of_pow_le
    (x A : Real) (n : Nat) (hx : 0 <= x) (hn : 0 < n)
    (hpow : x ^ n <= A) :
    x <= A ^ (1 / (n : Real)) := by
  have hnReal : 0 < (n : Real) := by exact_mod_cast hn
  have hroot := Real.rpow_le_rpow (pow_nonneg hx n) hpow
    (by positivity : 0 <= 1 / (n : Real))
  calc
    x = x ^ (1 : Real) := by rw [Real.rpow_one]
    _ = x ^ ((n : Real) * (1 / (n : Real))) := by
      congr 1
      field_simp
    _ = (x ^ (n : Real)) ^ (1 / (n : Real)) := by
      rw [Real.rpow_mul hx]
...
noncomputable def linnikVMTechSourcePowerRHS
...
theorem exists_linnik_vmtech_double_sum_source_root_bound :
    exists C : Real, And (0 < C) (forall
      (k r X L : Nat) (alpha theta : Fin k -> Real)
      (a q : Fin k -> Nat),
      2 <= k -> 0 < r -> 0 < X -> 0 < L ->
      ...
      (forall j,
        alpha j = (a j : Real) / q j + theta j / (q j : Real) ^ 2) ->
      norm (linnikVMTechDoubleSum alpha L X) <=
        (linnikVMTechSourcePowerRHS C r X L q) ^
          (1 / (((2 * (k * r)) * (2 * (k * r)) : Nat) : Real))) := by
  choose C hC hsource using exists_linnik_vmtech_double_sum_power_source_bound
  refine Exists.intro C (And.intro hC ?_)
  intro k r X L alpha theta a q hk hr hX hL hq hcop htheta halpha
  ...
  simpa only [linnikVMTechSourcePowerRHS] using hpower
